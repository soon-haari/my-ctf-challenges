class myAES:
    def __init__(self, master_key):
        assert len(master_key) == self.blocksize
        self.rows = self.blocksize // 4
    ...
    def _expand_key(self, master_key):
        ...
        i = 1
        while len(key_columns) < (self.n_rounds + 1) * self.rows:
        ...
        return [key_columns[self.rows*i : self.rows*(i+1)] for i in range(len(key_columns) // self.rows)]
    ...
    def encrypt(self, plaintext):
        assert len(plaintext) == self.blocksize
...
if __name__ == '__main__':
    ...
    for _ in range(10):
        key = os.urandom(16)
        msg = os.urandom(16)
        assert AES.new(key, AES.MODE_ECB).encrypt(msg) == myAES(key).encrypt(msg)
        assert AES.new(key, AES.MODE_ECB).decrypt(msg) == myAES(key).decrypt(msg)
